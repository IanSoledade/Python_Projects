...
gameStarted = False
score = 0
highscore = open('High Scores - DO NOT OPEN/highScore3.txt', 'r')
...
def startGame():
  ...
  while True:
    if outsideWindow():
      gameOver()
      break
    if s.distance(l) < (20 + (sLength * 10)):
      placeLeaf()
      sLength += 1
      s.shapesize(sLength, sLength, sLength)
      s.speed += 1
      score += 10
      if score >= int(hscore):
        highScore = open('highScore3.txt', 'w')
        hscore = score
        highScore.write(str(hscore))
        highScore.close()
      displayScore()
    elif s.distance(m) < (20 + (sLength * 10)):
      placeMega()
      sLength += 2
      s.shapesize(sLength, sLength, sLength)
      s.speed += 1.5
      ...
      if score >= int(hscore):
        ...
        highScore.close()
      displayScore()
    elif s.distance(u) < (20 + (sLength * 10)):
      placeUltra()
      sLength += 3
      s.shapesize(sLength, sLength, sLength)
      s.speed += 2
      score += 100
      if score >= int(hscore):
        highScore = open('High Scores - DO NOT OPEN/highScore3.txt', 'w')
        hscore = score
        highScore.write(str(hscore))
        highScore.close()
      displayScore()
